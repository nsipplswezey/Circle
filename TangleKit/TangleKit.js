...
//               data-stepwidth (optional): pixels to drag per step
...
Tangle.classes.TKAdjustableNumber = {
    ...
    initialize: function (element, options, tangle, variable) {
        ...
        this.stepwidth = (options.stepwidth !== undefined) ? parseFloat(options.stepwidth) : 5;
    ...
    initializeDrag: function () {
        ...
        var touchable = new BVTouchable(this.element, this);
        touchable.setHoverable(true);
    },
    ...
    touchDidMove: function (touches) {
        var value = this.valueAtMouseDown + touches.translation.x / this.stepwidth * this.step;
        value = ((value / this.step).round() * this.step).limit(this.min, this.max);
        this.tangle.setValue(this.variable, value);
        this.updateHelp();
        
        // once safari goes into Core Animation mode, it gets completely retarded and forgets
        // how to redraw properly. This is an awful hack to trick safari into redrawing.
        if (Browser.safari) {
            this.k = (this.k || 0) + 1;
            this.element.setStyle("backgroundColor", (this.k % 2) ? "transparent" : "#fff");
            this.element.parentElement.setStyle("color", (this.k % 2) ? "#696969" : "#686868");
        }
    },
